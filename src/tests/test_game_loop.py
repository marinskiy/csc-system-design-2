...
def test_player_makes_move(loop: GameLoop) -> None:
    ...
    assert isinstance(list(new_state.environment.map.get_objects(MapCoordinates(10, 9)))[0], PlayerCharacter)
    assert len(list(new_state.environment.map.get_objects(MapCoordinates(10, 10)))) == 0

    new_state = loop.run_game_turn(Key.A)
    assert isinstance(list(new_state.environment.map.get_objects(MapCoordinates(9, 9)))[0], PlayerCharacter)
    assert len(list(new_state.environment.map.get_objects(MapCoordinates(10, 9)))) == 0

    new_state = loop.run_game_turn(Key.S)
    assert isinstance(list(new_state.environment.map.get_objects(MapCoordinates(9, 10)))[0], PlayerCharacter)
    assert len(list(new_state.environment.map.get_objects(MapCoordinates(9, 9)))) == 0

    new_state = loop.run_game_turn(Key.D)
    assert isinstance(list(new_state.environment.map.get_objects(MapCoordinates(10, 10)))[0], PlayerCharacter)
